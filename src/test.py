...
class IPM(object):
    ...
    def __init__(self, camera_info, ipm_info):
        ...
        uv_limits = self.uv_limits = np.array([[ipm_info.left, ipm_top],
                              ...
                              [vp[1], ipm_top],
                              [vp[1], ipm_info.bottom]]).T # the limits of the area on the uv map to be IPM-converted
                              #[vp[0], ipm_top],
...
if __name__ == "__main__":
    ...
    camera_info = _DictObjHolder({
        "f_x": 718.856,         # focal length x
        "f_y": 718.856,         # focal length y
        "u_x": 607.1928,             # optical center x
        "u_y": 185,             # optical center y
        "camera_height": 1.65,  # camera height in `mm`
